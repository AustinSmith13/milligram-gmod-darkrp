...
if (SERVER) then
	...
	util.AddNetworkString("openprofile")
...
------------------------------ Fall_To_Death ------------------------------
function ulx.makefall( calling_ply, target_plys )
	local success_plys = {}
	for i=1, #target_plys do
		...
		if v:IsTerror() then
			...
			table.insert(success_plys, v)
		end
	end
	ulx.fancyLogAdmin( calling_ply, "#A made #T fall to their death.", success_plys)
end

local makefall = ulx.command( CATEGORY_NAME, "ulx makefall", ulx.makefall, {"!makefall", "!fall"} )
makefall:addParam{ type=ULib.cmds.PlayersArg, default="^", ULib.cmds.optional }
makefall:defaultAccess( ULib.ACCESS_ADMIN )
makefall:help( "Launches target to their death." )

------------------------------ Trainfuck ------------------------------
local traintable = {
{"models/props/de_train/diesel.mdl", 0, 0},
{"models/props_trainstation/train001.mdl", 90, 85},
{"models/props_combine/combine_train02a.mdl", 90, 105},
}
-- MODEL, ANGLEADJUST, HEIGHTADJUST

local function TrainFuck( ply, owner )
	local p = ply:GetPos() + ply:GetForward() * 5000
	if not util.IsInWorld(p) then
		local p = ply:GetPos() + ply:GetForward() * 4000
		if not util.IsInWorld(p) then
			local p = ply:GetPos() + ply:GetForward() * 3000
			if not util.IsInWorld(p) then
				local p = ply:GetPos() + ply:GetForward() * 2000
				if not util.IsInWorld(p) then
					local tr = util.TraceLine{start = ply:GetShootPos(), endpos = ply:GetPos() + ply:GetForward() * 5000, filter = ply, MASK_ALL}
					p = tr.HitPos
					-- last resort
				end
			end
	...
	local Pos, Direction = p, ply:GetForward() * -1

	local train = ents.Create( "prop_physics" )
	local tbl = table.Random(traintable)
	train:SetModel(tbl[1])
	train:SetAngles( Direction:Angle() )
	Pos.z = Pos.z + tbl[3]
	train:SetPos( Pos )
	train:Spawn()
	ply:EmitSound( "ambient/alarms/train_horn2.wav", 100, 100 )
	train:GetPhysicsObject():SetVelocity( Direction * 10000 )
	local ang = ply:GetAngles()
	ang:RotateAroundAxis(ang:Up(), tbl[2]) 
	train:SetAngles(ang)
	...
	timer.Simple(5, function() if not IsValid(train) then return end train:Remove() end)
end

hook.Add("ShouldCollide", "TrainShouldCollide", function(train, ent)
    if train.IsTrain then
        if ent == train.Target then
            return true
        else
            return false
        end
